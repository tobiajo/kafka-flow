...
object KafkaPartitionPersistence {
  ...
  /** Default no-progress deadline for the transactional recovery read; 3 minutes clears both configuration bounds at
    * Kafka defaults - see `TransactionalConfig.recoveryStallTimeout`.
    */
  val DefaultStallTimeout: FiniteDuration = 3.minutes
  ...
  private[kafkapersistence] def readSnapshots[F[_]: BracketThrowable: Log](
    ...
    // endOffsets is the last-stable-offset, which an open transaction pins below records committed after it -
                  ...
                  Log[F]
                    .warn(
                      s"Snapshot topic $snapshotTopic partition $partition recovery waits for open transaction(s): " +
                        s"last-stable-offset $lastStableOffset below target $highWatermark; normally resolves " +
                        s"within the pinning producer's transaction.timeout.ms plus the broker's abort scan"
                    )
