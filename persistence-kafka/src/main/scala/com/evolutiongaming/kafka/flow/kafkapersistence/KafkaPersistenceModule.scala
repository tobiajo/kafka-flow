...
object KafkaPersistenceModule {
  ...
  /** Settings for [[cachingTransactional]].
    ...
    *   both bounds are warned at module acquisition. The former is checked against this `consumerConfig`'s
    *   `maxPollInterval` as a stand-in - the module does not receive the driving consumer's config.
    */
  ...
  // the broker abort scan runs every transaction.abort.timed.out.transaction.cleanup.interval.ms, 10s by default
  private val brokerAbortScanInterval: FiniteDuration = 10.seconds
  ...
  def cachingTransactional[F[_]: LogOf: Async: Parallel: Runtime, S](
    ...
    for {
      ...
      // warn when configuration defeats the stall deadline (see TransactionalConfig.DefaultRecoveryStallTimeout)
      _ <- Resource.eval(
        log
          .warn(
            s"recoveryStallTimeout ${config.recoveryStallTimeout} is not below maxPollInterval " +
              ...
              "driving consumer's max.poll.interval.ms, which the module does not receive): a stalled recovery " +
        ...
        log
          .warn(
            s"recoveryStallTimeout ${config.recoveryStallTimeout} is not above transaction.timeout.ms " +
              ...
              s"($brokerAbortScanInterval by broker default; the broker's actual setting is not client-readable): " +
              "recovery waiting out an open transaction would fail even though the wait resolves on its own"
          )
          .whenA(
            config.recoveryStallTimeout <=
              config.producerConfig.transactionTimeout + brokerAbortScanInterval
          )
      )
  ...
  private def transactionalWriteDatabase[F[_]: Async, S](
    ...
    for {
      ...
      // transaction a crashed predecessor left open (no client-visible signal - the logged duration is its only trace)
