...
object KafkaPersistenceModule {
  ...
  /** Variant of `caching` protecting the snapshot topic from stale writers by binding the input-offset commit into the
    ...
    * Works under whichever protocol the driving consumer uses — both the classic and the consumer group protocols
    * (`group.protocol=classic|consumer`, the consumer protocol being KIP-848); use brokers 4.3.0+ for the consumer
    * protocol. See the "Protecting against stale snapshot writes" persistence docs for guarantees, limitations, costs
    * and rollout, and `docs/kafka-single-writer-design.md` for the mechanism.
    *
