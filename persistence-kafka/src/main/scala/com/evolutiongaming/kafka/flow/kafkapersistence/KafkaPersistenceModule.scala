...
object KafkaPersistenceModule {
  ...
  /** Settings for [[cachingTransactional]].
    ...
    *   fencing (that is by consumer generation), so its only roles are a readable label and, on an ACL-secured cluster,
    *   the `transactional.id` prefix the producer principal must be authorized for. Use your `applicationId`; an
    *   application running several flows can append any per-flow discriminator (e.g. the input topic), which an
