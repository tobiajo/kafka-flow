...
/** Pins the recovery-read behaviour: the target is the high watermark, not the read consumer's own end offset; a read
  * making no progress for the whole stall deadline fails loudly - diagnosed, and measured from the last advance, never
  * from the start of the read - while without a deadline (the non-transactional read) it keeps waiting.
  */
class ReadSnapshotsSpec extends FunSuite {
  ...
  test("a slow but progressing read outlives the deadline without failing") {
    ...
    // over it; the read must complete, not fail
  ...
  test("recovery warns about the wait when the target sits above the last-stable-offset") {
    ...
    val test = for {
      ...
      assert(
        warns.head.contains(s"last-stable-offset ${Offset.unsafe(1)} below target ${Offset.unsafe(3)}"),
  ...
  test("with no deadline a stalled read keeps waiting, and read_uncommitted opens no capture consumer") {
    ...
    // polling a virtual hour in - and under read_uncommitted the consumer's own end offset already is the
