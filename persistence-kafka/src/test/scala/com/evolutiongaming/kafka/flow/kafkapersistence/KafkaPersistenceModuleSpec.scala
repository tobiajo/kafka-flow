...
/** The transactional module owns the producer settings that carry its design: the stable per-partition
  * `transactional.id` (a takeover must abort a crashed owner's dangling transaction), idempotence, and the 10 s
  * `transaction.timeout.ms` (the Kafka Streams EOS default) - applied over whatever `producerConfig` carries.
  */
class KafkaPersistenceModuleSpec extends FunSuite {

  implicit val logOf: LogOf[IO] = LogOf.empty[IO]

  test("the module applies the stable per-partition id, idempotence and the 10 s transaction timeout") {
    val test = for {
