...
/** Verifies what actually happens when `commitTransaction` fails AMBIGUOUSLY (a client-side timeout: the broker may
  ...
  *
  * kafka-clients (4.3.0, read from source) keeps an unacked `pendingTransition` for the timed-out
  * `commitTransaction` (`TransactionManager.handleCachedTransactionRequestResult`); until that commit is retried to
  * completion every OTHER transactional operation on the producer throws
  * `IllegalStateException("Cannot attempt operation `X` because the previous call to `commitTransaction` timed out
  * and must be retried")`. So the blind abort:
  *
  ...
  *     `IllegalStateException` until either `commitTransaction` is retried on this instance or the producer is
  *     rebuilt.
  *
  * The Java client's contract (KafkaProducer.commitTransaction javadoc): "It is safe to retry in either case, but it
  * is not possible to attempt a different operation (such as abortTransaction) since the commit may already be in the
  ...
  * the real `KafkaSnapshotWriteDatabase.transactional` machinery and asserts the fixed behaviour: an ambiguous commit
  * is retried (never blindly aborted), so the write succeeds once the broker answers and the producer stays usable.
  *
...
class AmbiguousCommitAbortSpec extends ForAllKafkaSuite {
        ...
        transactionalProducer(s"$group-tx").use { producer =>
          val transaction = for {
            ...
            _ <- producer
              ...
              .flatten
              .void
            _ <- producer.sendOffsetsToTransaction(NonEmptyMap.of(inputTp -> OffsetAndMetadata(Offset.unsafe(42))), meta)
  ...
  test("GroupCommit resolves an ambiguous commit by retrying it: the write succeeds and the producer stays usable") {
      ...
      _ <- withJoinedConsumer(group, inputTopic) { meta =>
        transactionalProducer(s"$group-tx").use { producer =>
          val scenario = for {
            ...
            // the first write's commit times out client-side (broker paused) - an ambiguous failure. The fixed
            // commitBatch retries the commit instead of blindly aborting, so once the broker is back the pending
            // EndTxn completes and the write SUCCEEDS.
            _ <- tx.writeDatabase.write(key, Stored.Live("value-1", none))
            // and the producer is still usable: the next transaction on the partition works
            _ <- tx.writeDatabase.write(key, Stored.Live("value-2", none))
            // both transactions landed - the later snapshot is the committed recovery value
            _ <- eventually("both snapshot writes are visible to read_committed") {
              readCommittedSnapshots(stateTopic).map(_.get(key.key).flatMap(utf8).filter(_ == "value-2"))
            }
          } yield ()
