...
went wrong; models prove which designs work; this file states the obligations forward — for a PR
author or reviewer to check against. Corpus index: [`README.md`](README.md).*
...
|---|---|---|
| Kafka transactional (merged, #833, EXPERIMENTAL) | in master and this branch | K-1..K-6, K-8 yes; **K-7 only half-met on master** — the `read_committed` *isolation* half is tested, but the *completeness* half is R-850, so F-10 (high) is still unfixed on the merged/master branch; **R-850 / R-849 open** (issues #850/#849). The remedies are open upstream drafts: **A** = the high-watermark read bound (#852), **B** = the stable per-partition id (#853), the **#849 stall deadline** (#851), each branch-verified alone. **A is required for full safety; B is optional, for post-crash recovery speed** ([Recommendation](850-remedy-decision.md#5-recommendation)); a combined A+B(+deadline) implementation (R-850-C) now exists as a single fork PR (tobiajo#16, `tj/recovery-read-complete-and-bounded`), carrying R-a and R-b below, unmerged — the models branch expects to pull it in if adopted. Release standing: **v9.0.0 is the only release carrying the mode** (the #833+#840 cut — F-10 live in it); #842, #854 and the remedies are unreleased. |
...
### R-850 — recovery-read completeness (issue #850, finding F-10) — **OPEN**
...
**Option A — high-watermark bound (PR #852, `recoveryread_hw_unique`).**
...
- **A-test.** An IT MUST hold a transaction genuinely open across the read and **assert the
  ...
  above the pin. *Currently on PR #852's branch, not here.*

**Option B — stable per-partition `transactional.id` (PR #853, `recoveryread_lso_stable`).**
...
- **R-849.2a.** The implementation SHOULD validate the R-849.2 inequalities at wiring time (fail
  ...
  eviction. **Status on the upstream draft (#851):** the **upper** bound is checked (a warn when
  `recoveryStallTimeout` is not below `max.poll.interval.ms`); the **lower** bound
...
- **R-849.3.** On trip the read MUST raise an error that fails the flow through normal supervision
  ...
  including a pre-v2 *hanging* transaction no timeout resolves (ext(K14)) — to route the operator's
  response (offset-reset/restore vs. wait/abort).
- **R-849-test.** The pin MUST be a client-side test that stalls progress (a consumer whose position
  ...
  documentation grade.
- **Upstream draft:** #851 (`tj/recovery-read-stall-tripwire`) — a wall-clock deadline
  ...
  models branch expects to pull it in if adopted (the report's [Open work](README.md#6-open-work)).
  The lower-bound check (R-a), the A×deadline coexistence test (R-b), and the trip diagnosis above
  are obligations a combined A+B+deadline implementation adds (R-850-C).
...
## P — Cassandra persist-only mode (merged upstream, #838)

The subset of C actually merged: persists fenced, deletes plain.
...
- **X-3:** `SnapshotWriteDatabase.delete` carries an offset (the #828-era breaking change) — a fenced
...
branches this session. What is missing is the merge, not the artifact — the A-vs-B choice is now made
in principle ([`850-remedy-decision.md`](850-remedy-decision.md): A required, B optional) and the
combined-corner obligations recorded (R-850-C above).
Everything else is implemented on its arm's branch with the named test.

## Integration gaps across the code branches (#851–#853)

Each upstream draft carries its code **and** a non-vacuous pinning test (R-849-test on #851; live +
unit A-test on #852; takeover-abort B-test + id-shape pin on #853). These are gaps *in the
combination / at the edges*, not defects in any single draft — the obligations a combined
A+B+deadline implementation must satisfy (collected as R-850-C above), stated here as requirements
because no single draft carries them:

- **R-a (code): validate the deadline's *lower* bound, not only the upper.** #851 warns when
...
- **TLC 2.18 re-run** — a toolchain refresh only; the suite is verified on the pinned release v1.7.0
  (self-reports 2.15, [`findings.md`](findings.md) ledger). Not a correctness task.
