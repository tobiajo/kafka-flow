...
as the work. Arm's-length in *context*, not in *authorship* — a rung below a human adversary. This is
the **same** mechanism as the Cassandra arm's F-7 "review committee" (which, read honestly, was also
fresh-context AI, not human — see finding **H1**). So both arms sit at parity: AI-reviewed, human
review still outstanding. That the review still found real defects (below) is the argument for doing
it; that it cannot substitute for a human adversary is the argument for not overclaiming it.
...
|---|---|---|---|
| **H1** | high | README called the Cassandra F-7 review a "**human committee**"; [`cassandra-report.md`](cassandra-report.md) §10 describes only a "fresh-context review committee" inside an "AI-assisted effort" — no evidence it was human. Introduced this session without basis; overclaims independence on the corpus's load-bearing F-7 catch. | **Fixed.** README §6 (Open work) reframed: both arms' external reviews are fresh-context **AI** passes at parity; a human pass is above both and neither has had it. |
| **M1** | med | Register epilogue said R-850/R-849 have "**no implementation anywhere … tests not yet written**" — contradicted by the same file, `outstanding-work.md`, [`findings.md`](findings.md), [`claims.md`](claims.md) (code+tests exist on draft branches, branch-verified). | **Fixed.** Epilogue now: open = **unmerged**, not unbuilt; each has code + a non-vacuous test on its draft branch. |
...
| **M3** | med | "**Theoretically complete / no open question remains**" overstates vs the open **C2** (one-handover cast, not inductive *n*). | **Fixed.** Bucket 1 now "complete modulo disclosed generality residuals (C2/C3)"; `outstanding-work.md` banner aligned. |
...
| **M6** | med | Stray "**2.16**" at `outstanding-work.md` (the corpus-wide correction to 2.15 missed one line), which also falsified "corrected across all 7 files." | **Fixed.** → 2.15 / release v1.7.0; the meta-claim reworded to "across the corpus." |
...
Kafka arm had had none — the standing gap in the report's [Open work](README.md#6-open-work). This pass closes
it. **What it is, honestly:** three independent fresh-context adversarial reviewers (no prior
...
authorship — a step below a *human* adversary (the same standing as the Cassandra arm's own
fresh-context committee; neither arm has had a human pass — see the report's [Open work](README.md#6-open-work) and
finding H1 above), but it attacked stated claims and probed
...
- *RecoveryRead* — non-vacuous (the `EndOffsetsIsLSO` flip changes the verdict with the reader code
  ...
  safety-under-truncation is thus mechanized, not prose. **(C2)** the cast is one A→B handover, not an
  inductive n; the lineage-serialization argument is representative, not general (still open).
...
arm's models mechanize, and they belong here rather than in silence. A human arm's-length review
remains the one rung above this pass; the report's Open work now reflects the AI pass as done with these
residuals rather than the review as absent.

## What remains open after this review

- **C2** (one-handover cast, not inductive *n*), **C3** (trip-abort latency zeroed) — disclosed
  residuals, not closed by this review.
- The **tower↔RecoveryRead seam** (prose implication-transitivity on the exact F-10 fault line) and
  the **capture divergence** (M5) — now recorded as gaps, still unmechanized.
- **Human** arm's-length review — the rung above this AI pass, which neither arm has had.
...
left as "flagged for a future pass." What remains open (above) is genuine forward work — the disclosed
generality residuals C2/C3, the unmechanized seams, the human review, and the implementation merges —
not review findings parked unaddressed.

Snapshot date: 2026-07-12 (keep in step with the report's [Open work](README.md#6-open-work)).
