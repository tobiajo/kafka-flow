...
each realized in a draft PR:

- **A — bounded wait** (PR #852): writer identity stays unique per assignment; the *reader*
...
- **B — takeover-abort** (PR #853): writer identity is stable per partition
...
branch's module shape, preserved by #853). So the race needs an acquisition in flight at a
...
measures. Note the ordering: under A+B the HW capture as coded in #852 runs *after* module
...
- Evidence that the two code paths' co-maintenance produces defects the single-mechanic corners
  ...
  is the pinned test pair: the wait IT (#852's, orphan outside the lineage) and the takeover IT
  (#853's) each fail if their mechanic regresses.
