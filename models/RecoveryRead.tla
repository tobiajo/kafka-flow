...
(*   - StableId : writers share the partition's transactional.id (candidate      *)
(*     PR #853), so a takeover's mandatory init aborts the predecessor's open    *)
...
(*     separate read_uncommitted lens (candidate PR #852; EndOffsetsIsLSO is     *)
...
(*   - recoveryread_hw_unique   HOLDS: remedy 1 (PR #852) -- the HW bound makes   *)
(*     the read WAIT the open transaction out (Kafka Streams' restore shape,      *)
(*     KAFKA-10167, ext(K6)).                                                     *)
(*   - recoveryread_lso_stable  HOLDS: remedy 2 (PR #853) -- B's mandatory init   *)
