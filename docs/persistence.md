...
`CommitFailedException` — or as a producer-epoch error when a new owner's `initTransactions` has
...
- **Recovery fails loudly rather than hangs** — a recovery read that makes no progress for
  ...
  Cluster-side, the matching broker alerts are `PartitionsWithLateTransactionsCount > 0` (hanging
  transactions) and `UncleanLeaderElectionsPerSec > 0` (truncation risk); brokers 4.1+ close the
  remaining routine paths to both causes. Consumer lag metrics read zero during the wait or stall
  (lag is measured to the last-stable-offset, where the read parks), so alert on this mode's log
  signals, not on lag. Keep the value well below `max.poll.interval.ms` and above
...
- A `transactionalIdPrefix` change can cost recovery a wait: an old-prefix instance that dies
  ...
  until the broker times it out — up to ~70 s at the defaults (`transaction.timeout.ms` plus the
  broker's abort scan), never a wrong read. A transaction is open only during a synchronous flush
  or offset commit, so a graceful rollout leaves nothing open. (A foreign producer's transaction on
  the snapshot topic is waited out the same way — but the topic must be exclusive to the flow
  regardless.)
