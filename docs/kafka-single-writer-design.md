...
a previous prefix's unfinished transactions while a prefix change rolls out, or a producer
misdirected at the snapshot topic — and there waiting is the only correct behavior, bounded by the
*pinning* producer's `transaction.timeout.ms` plus the broker's abort scan
...
unreachable. Kafka has closed the routine paths to this
...
min, measured from the last position advance) fails the read loudly. The wait it must outlast is
...
and the restarted recovery captures a fresh target — after a truncation it completes, but over the
shortened log, so the loss stands until an operator acts (below); behind a hanging transaction it
fails loudly again until the pin is cleared.
...
real eager-recovery (every key recovered on assignment) and flush-on-revoke machinery:

- **Reproduction and prevention** — corruption shown with the plain shared producer (no offset
  binding); prevention drives a stale owner with an *older consumer generation* and asserts the
  newer snapshot survives. Under the stable id that stale flush dies at the epoch fence (the new
  owner's init has already fenced it), so the generation fence itself is isolated by the fail-fast
  and offset-commit tests, which drive a live, un-fenced producer whose generation alone is stale.
- **Write path** — first-flush gating (the seed), a fenced writer fails its next flush,
  concurrent-write safety.
- **Unfinished transactions, both resolutions** — the takeover-abort pinned at the handover (the
...
- **`GroupCommitSpec`** — the group commit in isolation, with a recording in-memory producer (no
  broker).
