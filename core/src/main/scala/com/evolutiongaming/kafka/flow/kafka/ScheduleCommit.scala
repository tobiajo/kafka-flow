...
/** Schedules an input offset to be committed for a partition. `schedule` is not necessarily a cheap in-memory record:
  ...
  * [[GenerationFencedError]], which the periodic path tolerates (the offset stays uncommitted and is scheduled again on
  * the next tick); any other failure fails the flow. On revoke every failure is best-effort (swallowed, since the
  * partition is being handed off anyway).
  */
