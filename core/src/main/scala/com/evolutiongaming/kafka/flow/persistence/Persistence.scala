...
object ReadState {
  ...
  /** Restores state from previously saved events, after seeding the snapshot buffer's replay-window floor from the
    ...
    * events-recovery alone cannot reconstruct a deleted key's high-water offset; reading the snapshot store surfaces the
    * deletion tombstone's offset as the floor. Without it a replayed event below that offset would be re-derived and
    * persisted, which a compare-and-set snapshot store rejects as stale though the owner is legitimate -- the
