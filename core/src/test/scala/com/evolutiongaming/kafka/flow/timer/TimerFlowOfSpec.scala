...
class TimerFlowOfSpec extends FunSuite {
  ...
  test("persistPeriodically tolerates a fenced persist even when ignorePersistErrors = false") {
    ...
    // the broker rejected the snapshot transaction for a stale consumer generation; the writer surfaces it as the
    // dedicated type and the timer flow must not fail on it
  ...
  List(
    "fenced"  -> (GenerationFencedError(new Exception("stale generation")), false),
    "ignored" -> (new Exception("Test error"), true),
  ).foreach {
    case (kind, (error, ignorePersistErrors)) =>
      test(s"persistPeriodicallyAndUnloadOrphaned keeps a key loaded when its persist is $kind") {

        val f = new ConstFixture

        // unloading also drops the held offset: an unpersisted key that is unloaded would let the partition commit
        // past state that never reached the store
        val failingFlushBuffers: FlushBuffers[IO] = new FlushBuffers[IO] {
        ...
        val testIO = for {
          _      <- f.contextRef.set(context)
          _      <- program(flowOf(f.keyContext, failingFlushBuffers, f.timerContext))
          result <- f.contextRef.get
        } yield {
          // Then("the key is neither flushed nor unloaded and still holds its offset")
          assertEquals(result.flushed, 0)
          assertEquals(result.removed, 0)
